...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = FreeIpOmni.make_runner(TEST_JSON_FILE, FreeIpSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : FreeIpOmni.errify(err)
    end
