...
class FreeIpSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FreeIpHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreeIpHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreeIpFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FreeIpHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FreeIpHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreeIpHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FreeIpHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FreeIpError.new(
        "graphql_error", "FreeIpSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.IpGeolocation.list / client.IpGeolocation.load({ "id" => ... })
  def IpGeolocation(data = nil)
    require_relative 'entity/ip_geolocation_entity'
    IpGeolocationEntity.new(self, data)
  end
