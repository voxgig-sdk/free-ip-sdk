...
class IpGeolocationEntity
  ...
  def make
    opts = @_entopts.dup
    IpGeolocationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FreeIpHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [IpGeolocation, Hash] the current IpGeolocation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FreeIpHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of IpGeolocation fields)
  def match_get
  ...
  #
  # @param reqmatch [IpGeolocationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.IpGeolocation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [IpGeolocation, Hash] the loaded IpGeolocation; raises FreeIpError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FreeIpHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
