# FreeIp Lua SDK Reference

Complete API reference for the FreeIp Lua SDK.
...
```lua
local ip_geolocation = client:IpGeolocation(nil)
```
...
```lua
local result, err = client:Json():create({
})
```
...
```lua
local results, err = client:Json():list()
```
