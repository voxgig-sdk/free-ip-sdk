...
local IpGeolocationEntity = {}
IpGeolocationEntity.__index = IpGeolocationEntity


function IpGeolocationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IpGeolocationEntity)
  self._name = "ip_geolocation"
  self._client = client
...
function IpGeolocationEntity:get_name()
  return self._name
end


function IpGeolocationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IpGeolocationEntity.new(self._client, opts)
end
...
function IpGeolocationEntity:mark_deleted()
  self._deleted = true
end


function IpGeolocationEntity:deleted()
  return true == self._deleted
end


function IpGeolocationEntity:data_set(args)
  if args ~= nil then
...
function IpGeolocationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IpGeolocationEntity:match_set(args)
  if args ~= nil then
...
function IpGeolocationEntity:match_get()
...
function IpGeolocationEntity:stream(action, args, callopts)
...
---@param reqmatch IpGeolocationLoadMatch
---@param ctrl? table
---@return IpGeolocation
---@return string? err
function IpGeolocationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function IpGeolocationEntity:_run_op(ctx, post_done)
  local utility = self._utility
