...
[MCP](https://modelcontextprotocol.io) server exposing the FreeIp SDK as
two agent tools — `free-ip_list` and `free-ip_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-ip-mcp)
make build

# 2. Provide credentials via the environment
export FREE_IP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-ip \
  -- /absolute/path/to/free-ip-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-ip-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-ip_list: first page of records
{ "entity": "json" }
{ "entity": "json", "query": { } }

// free-ip_load: one record by id
{ "entity": "ip_geolocation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-ip-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-ip \
     -- "$PWD"/dist/*/free-ip-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-ip_list` and `free-ip_load` tools now appear
   in new sessions. Ask the agent to *"list json using free-ip"*
   and it calls `free-ip_list` with `{"entity":"json"}`.
...
```sh
export FREE_IP_APIKEY=sk_live_xxx            # API key
export FREE_IP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `free-ip_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-ip_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_IP_APIKEY` | API key sent with every request. |
| `FREE_IP_BASE` | Optional override of the API base URL. |
...
```sh
./free-ip-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-ip_load","arguments":{"entity":"ip_geolocation","query":{"id":1}}}}'
```
