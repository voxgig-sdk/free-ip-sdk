...
{
    private static function utility(): FreeIpUtility
    {
        return new FreeIpUtility();
    }
    ...
    private static function ctx(array $over = []): FreeIpContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FreeIpContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FreeIpOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FreeIpError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FreeIpOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FreeIpMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FreeIpOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FreeIpMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FreeIpOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FreeIpMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeIpSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FreeIpMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FreeIpResponse([]),
            'result' => new FreeIpResult([]),
        ]);
        [, $err] = FreeIpMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FreeIpSpec(['step' => 's']),
            'response' => new FreeIpResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FreeIpResult([]),
        ]);
        [, $err] = FreeIpMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeIpSpec(['step' => 's']),
            'response' => new FreeIpResponse(self::resp(200, ['v' => 1])),
            'result' => new FreeIpResult([]),
        ]);
        [, $err] = FreeIpMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeIpSpec(['step' => 's']),
            'response' => new FreeIpResponse(self::resp(200, ['v' => 2])),
            'result' => new FreeIpResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FreeIpMakeResponse::call($ctx);
    ...
    {
        $preset = new FreeIpResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FreeIpSpec([]),
            'response' => new FreeIpResponse([]),
            'result' => new FreeIpResult([]),
        ...
        [$got, $err] = FreeIpMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FreeIpResult([])]);
        [, $err] = FreeIpMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FreeIpSpec([]), 'result' => null]);
        [, $err] = FreeIpMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeIpSpec(['step' => 's']),
            'result' => new FreeIpResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FreeIpOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeIpMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeIpSpec(['step' => 's']),
            'result' => new FreeIpResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FreeIpOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeIpMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeIpResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FreeIpSpec([]),
            'result' => new FreeIpResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FreeIpMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeIpMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FreeIpError('boom', 'boom');
        $utility->fetcher = function (FreeIpContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeIpSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FreeIpMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FreeIpContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FreeIpMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FreeIpResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FreeIpContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeIpSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FreeIpMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FreeIpResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FreeIpSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FreeIpMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FreeIpMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FreeIpMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FreeIpResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FreeIpDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FreeIpResult(['ok' => false])]);
        $this->expectException(FreeIpError::class);
        FreeIpDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FreeIpResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FreeIpMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FreeIpBaseFeature();
        $b = new FreeIpBaseFeature();
        FreeIpFeatureAdd::call($ctx, $a);
        FreeIpFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FreeIpBaseFeature
    {
        $f = new FreeIpBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FreeIpFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FreeIpContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FreeIpSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeIpPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FreeIpResponse(['headers' => null]),
            'result' => new FreeIpResult([]),
        ]);
        FreeIpResultHeaders::call($ctx);
