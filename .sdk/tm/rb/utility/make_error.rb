...
module FreeIpUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = FreeIpContext.new({}, nil)
    end
    op = ctx.op || FreeIpOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || FreeIpResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(FreeIpError) ? err.msg : err.to_s
    ...
    sdk_err = FreeIpError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(FreeIpError)
