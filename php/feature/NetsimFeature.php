...
class FreeIpNetsimFeature extends FreeIpBaseFeature
{
    ...
    private function _simulate(FreeIpContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FreeIpContext $ctx, array $applied): void
    {
        $client = $this->client;
