...
class FreeIpMakeError
{
    public static function call(?FreeIpContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FreeIpContext([], null);
        }
        $op = $ctx->op ?? new FreeIpOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FreeIpResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FreeIpError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FreeIpError('', $msg, $ctx);
