...
class FreeIpSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FreeIpHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FreeIpHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FreeIpFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FreeIpFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FreeIpUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FreeIpHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FreeIpHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FreeIpHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FreeIpError($op . "_allow",
                "FreeIpSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FreeIpHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FreeIpError("graphql_error",
                "FreeIpSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_ip_geolocation = null;

    // Canonical facade: $client->IpGeolocation()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->ip_geolocation()
    // resolves here too.
    public function IpGeolocation($data = null)
    {
        require_once __DIR__ . '/entity/ip_geolocation_entity.php';
        if ($data === null) {
            if ($this->_ip_geolocation === null) {
                $this->_ip_geolocation = new IpGeolocationEntity($this, null);
            }
            return $this->_ip_geolocation;
        }
        return new IpGeolocationEntity($this, $data);
    }
