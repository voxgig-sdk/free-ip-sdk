...
// FreeIp SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FreeIpOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(FreeIpOmni::structfix($now, Struct::undef()))
                        !== json_encode(FreeIpOmni::structfix($val, Struct::undef()))) {
