...
class FtCtrl extends FreeIpControl
{
...
class FtClient
{
    ...
    private FreeIpUtility $utility;

    public function __construct(FreeIpUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeIpUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeIpContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeIpContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeIpConfig::make_feature($name);
    ...
    private static function buildUrl(FreeIpSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeIpError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeIpResponse($fetched);
        }
    ...
    private function fail(FreeIpContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeIpContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
