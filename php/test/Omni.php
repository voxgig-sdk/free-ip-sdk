...
//    only - a typed FreeIpContext there would make every ctx
...
final class FreeIpOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new FreeIpContext($clean, null);
    ...
    public static function err_from_map($m): ?FreeIpError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FreeIpError(is_string($code) ? $code : '', $msg);
    }
