...
class JsonEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = FreeIpSDK::test($seed, null);
        $seen = iterator_to_array($base->Json(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = FreeIpConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = FreeIpSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Json(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FREEIP_TEST_JSON_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function json_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("FREEIP_TEST_JSON_ENTID");
