...
// FreeIp SDK IpGeolocation entity
...
class IpGeolocationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new IpGeolocationEntity($this->_client, $opts);
    }

    /**
     * @param IpGeolocation|array $args IpGeolocation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FreeIpHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return IpGeolocation|array The current IpGeolocation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of IpGeolocation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FreeIpHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of IpGeolocation fields).
     */
    ...
    /**
     * Load a single IpGeolocation.
     *
     * @param IpGeolocationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed IpGeolocationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return IpGeolocation|array The loaded IpGeolocation as an assoc-array at the
     *   SDK boundary; throws FreeIpError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FreeIpHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
