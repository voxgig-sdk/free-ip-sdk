...
The PHP SDK for the FreeIp API — an entity-oriented client using PHP conventions.
...
try {
    ...
    print_r($ipgeolocation->data_get());
} catch (\Throwable $err) {
...
$json = $client->Json()->list();
print_r(array_map(fn($item) => $item->data_get(), $json));
```
...
| `currency` | `array` | Currency information for the country |
| `id` | `string` |  |
...
```php
// list() returns an array of Json records (throws on error).
...
The main class (`freeip_sdk.php`) exports the SDK class
