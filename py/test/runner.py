...
class FreeIpTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeIpTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeIpTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeIpTestRunner.getenv("FREE_IP_TEST_LIVE")
        override = FreeIpTestRunner.getenv("FREE_IP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeIpTestRunner.getenv(key)
        ...
        explain = FreeIpTestRunner.getenv("FREE_IP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREE_IP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreeIpTestRunner._test_control is not None:
            return FreeIpTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeIpTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeIpTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeIpTestRunner.load_test_control()
...
def load_env_local():
    FreeIpTestRunner.load_env_local()


def env_override(m):
    return FreeIpTestRunner.env_override(m)


def entity_data(v):
    return FreeIpTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeIpTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeIpTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeIpTestRunner.load_test_control()


def live_client_options():
    return FreeIpTestRunner.live_client_options()


def live_delay_ms():
    return FreeIpTestRunner.live_delay_ms()
