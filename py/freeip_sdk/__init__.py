...
from freeip_sdk.utility import register

# Load features
from freeip_sdk.feature.base_feature import FreeIpBaseFeature
from freeip_sdk.features import _has_feature, _make_feature
...
class FreeIpSDK:
    ...
    def get_utility(self):
        return FreeIpUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("FreeIpSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def IpGeolocation(self, data=None) -> "IpGeolocationEntity":
        """Entity factory: client.IpGeolocation().list() / client.IpGeolocation().load({"id": ...})."""
        from freeip_sdk.entity.ip_geolocation_entity import IpGeolocationEntity
        return IpGeolocationEntity(self, data)


    def Json(self, data=None) -> "JsonEntity":
        """Entity factory: client.Json().list() / client.Json().load({"id": ...})."""
        from freeip_sdk.entity.json_entity import JsonEntity
        return JsonEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "FreeIpSDK":
        if sdkopts is None:
            sdkopts = {}
