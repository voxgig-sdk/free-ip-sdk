...
from freeip_sdk.freeip_types import (
    IpGeolocation,
...
class IpGeolocationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IpGeolocationEntity(self._client, opts)
    ...
    def data_get(self) -> IpGeolocation:
    ...
    def match_get(self) -> IpGeolocation:
    ...
    def load(self, reqmatch=None, ctrl=None) -> IpGeolocation:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.IpGeolocation().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
