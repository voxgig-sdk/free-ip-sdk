...
describe('IpGeolocationDirect', async () => {
  ...
  test('direct-load-ip_geolocation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ip_geolocation', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FREE_IP_TEST_IP_GEOLOCATION_ENTID']
