# FreeIp TypeScript SDK



The TypeScript SDK for the FreeIp API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.IpGeolocation()` — each with a small set of operations (`list`, `load`, `create`)
...
| `tester(testopts?, sdkopts?)` | `FreeIpSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `FreeIpSDK.test(testopts?, sdkopts?)` | `FreeIpSDK` | Create a test-mode client. |
...
| `client` | `client(): FreeIpSDK` | Return the parent SDK client. |
...
Create an instance: `const ip_geolocation = client.IpGeolocation()`
...
```ts
const jsons = await client.Json().list()
```

#### Example: Create

```ts
const json = await client.Json().create({
})
```
