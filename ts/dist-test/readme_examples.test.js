...
const TEST_SEED = { "entity": { "ip_geolocation": { "test01": { "id": "test01" } }, "json": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+FreeIpSDK\b/.test(code) ||
        /\bFreeIpSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+FreeIpSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/FreeIpSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
