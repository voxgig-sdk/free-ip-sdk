...
boru-driven command-line client **and** interactive REPL for the FreeIp
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-ip-cli)
make build
...
./free-ip-cli load 1 ip_geolocation            # {id:1} shorthand
./free-ip-cli load '{id:1}' ip_geolocation       # explicit match map

# 5. Override the API base URL for a single call
FREE_IP_BASE=https://api.example.com ./free-ip-cli load 1 ip_geolocation

# 6. No arguments -> interactive REPL
./free-ip-cli
free-ip> load 1 ip_geolocation
free-ip> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-ip-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-ip-cli load 1 ip_geolocation
   ```
...
```sh
./free-ip-cli load 1 ip_geolocation          # scalar shorthand for {id:1}
./free-ip-cli load '{id:1}' ip_geolocation     # explicit match map
```
...
./free-ip-cli load 1 ip_geolocation
```
...
Run with no arguments to open a REPL (prompt `free-ip>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `ip_geolocation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-ip-cli`. |
...
(`./free-ip-cli <expr>`) and interactively (the REPL), and expressions compose the
