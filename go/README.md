...
The Golang SDK for the FreeIp API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.IpGeolocation(nil)` — each with the same small set of operations (`List`, `Load`, `Create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
## Install
```bash
go get github.com/voxgig-sdk/free-ip-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/free-ip-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/free-ip-sdk/go=../free-ip-sdk/go
```
...
fmt.Println(ipGeolocation) // the returned mock data
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *FreeIpSDK
```
...
| `IpGeolocation` | `(data map[string]any) FreeIpEntity` | Create an IpGeolocation entity instance. |
| `Json` | `(data map[string]any) FreeIpEntity` | Create a Json entity instance. |

### Entity interface (FreeIpEntity)

All entities implement the `FreeIpEntity` interface.
...
slice):
    ...
    if err != nil { /* handle */ }
    // ipGeolocation is the returned record
...
Create an instance: `ipGeolocation := client.IpGeolocation(nil)`
...
fmt.Println(ipGeolocation) // the loaded record
```
...
| `currency` | `map[string]any` | Currency information for the country |
| `id` | `string` |  |
...
```go
jsons, err := client.Json(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(jsons) // the array of records
```
...
```
github.com/voxgig-sdk/free-ip-sdk/go/
├── free-ip.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/free-ip-sdk/go`) re-exports everything needed
