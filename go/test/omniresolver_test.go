...
import (
	...
	omni "github.com/voxgig-sdk/free-ip-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeIpError); is {
				out := map[string]any{
					"name":    "FreeIpError",
...
func omniCtx(arg any, client *sdk.FreeIpSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeIpSDK); is {
			client = live
