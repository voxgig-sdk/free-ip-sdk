...
func TestJsonEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Json(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Json(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREE_IP_TEST_JSON_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		jsonRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "json"}, setup.data), "json_ref01"))

		jsonRef01DataResult, err := jsonRef01Ent.Create(jsonRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		jsonRef01Data = core.ToMapAny(entityData(jsonRef01DataResult))
		...
		jsonRef01ListResult, err := jsonRef01Ent.List(jsonRef01Match, nil)
		if err != nil {
...
func jsonBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREE_IP_TEST_JSON_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREE_IP_TEST_JSON_ENTID"])
