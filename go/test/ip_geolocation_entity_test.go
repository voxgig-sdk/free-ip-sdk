...
func TestIpGeolocationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREEIP_TEST_IP_GEOLOCATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func ip_geolocationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREEIP_TEST_IP_GEOLOCATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREEIP_TEST_IP_GEOLOCATION_ENTID"])
